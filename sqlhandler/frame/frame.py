...
import io
import os
from typing import Any, Collection, Union, Type, Iterable, TypeVar, Callable
import pathlib
...
class Frame(pd.DataFrame):
    ...
    DEFAULT_COLUMN_CASE = Str.Case.SNAKE
    ...
    def to_sql(self, engine: Any, name: str, if_exists: Enums.IfExists = Enums.IfExists.FAIL, index: bool = True, index_label: str = "id", primary_key: str = "id", schema: str = None, dtype: dict = None, **kwargs: Any) -> None:
    ...
    def to_table(self, table: str, schema: str = None, database: str = None, if_exists: Enums.IfExists = Enums.IfExists.FAIL, primary_key: str = "id", sql: Any = None) -> Any:
    ...
    def sanitize_colnames(self, casing: Str.Case = None) -> Frame:
        ...
        df.columns = [Str(colname).case.from_enum(casing) for colname in df.columns]
    ...
    def _get_path_constructor(cls) -> Callable[..., PathLike]:
        if cls.DEFAULT_PATH_TYPE == Enums.PathType.PATHMAGIC:
            from pathmagic import File
            return File.from_pathlike
        elif cls.DEFAULT_PATH_TYPE == Enums.PathType.PATHLIB:
            return pathlib.Path
        elif cls.DEFAULT_PATH_TYPE == Enums.PathType.STRING:
            return os.fspath
        else:
    ...
    def _infer_range(self, mode: Enums.InferRange = None) -> None:
        if mode is None:
            pass
    ...
    def _decrypted_stream_from_xlsx(path: PathLike, password: str) -> io.BytesIO:
        ...
        with open(path, "rb") as source_stream:
            ...
            file.decrypt(destination_stream := io.BytesIO())
